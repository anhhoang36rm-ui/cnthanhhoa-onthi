...
# Nếu chạy sau reverse proxy (Nginx/Caddy...) có HTTPS, đặt biến môi trường TRUST_PROXY=1
# để Flask nhận đúng thông tin proto/IP gốc từ header X-Forwarded-*.
...
def _cookie_is_secure():
    """True nếu request hiện tại là HTTPS (trực tiếp hoặc qua reverse proxy có TRUST_PROXY=1)."""
    try:
...
def set_app_cookie(resp, key, value, max_age):
    """Đặt cookie với các cờ bảo mật: HttpOnly (JS không đọc được), SameSite=Lax (chống CSRF cơ bản),
    Secure tự động bật khi phục vụ qua HTTPS."""
    resp.set_cookie(key, value, max_age=max_age, httponly=True, samesite="Lax", secure=_cookie_is_secure())


@app.after_request
def set_security_headers(resp):
    """Thêm các header bảo mật cơ bản cho mọi phản hồi (chống clickjacking, MIME sniffing...)."""
...
# Mật khẩu quản trị: lưu bền vững ra file để admin có thể tự đổi mật khẩu.
# Nếu file chưa tồn tại, dùng mật khẩu mặc định "admin123".
...
# Chống dò mật khẩu (brute-force): giới hạn số lần đăng nhập sai theo IP + email trong 1 khoảng thời gian.
LOGIN_ATTEMPTS = {}
...
# Lưu bền vững (ra file JSON) các câu hỏi đã thi của từng tài khoản cho từng bộ đề,
# dùng cho chế độ "Không lặp câu đã thi trong phiên đăng nhập".
# Cấu trúc trong bộ nhớ: { email: { ten_file: set(id_cau_hoi_da_thi) } }
# Dữ liệu này được ghi ra file nên vẫn còn nguyên dù server khởi động lại,
# và chỉ bị xóa khi người dùng chủ động bấm Đăng xuất.
...
"""
# danh sách email quản trị, phân tách bằng dấu phẩy (ví dụ: admin)
...
.body {
  ...
  position:relative; /* giữ khung cố định và cho phép form chồng lên nhau */
}
.section-title {
  margin:0 0 16px;
  font-size:19px;
...
input {
  ...
  border-radius:10px;
  font-size:16px; /* >=16px tránh iOS tự động zoom khi focus */
  color:var(--text);
  background:#fff;
}
...
/* form panels: chỉ hiện panel đang active, panel còn lại ẩn hoàn toàn
   (không dùng absolute positioning để chiều cao luôn khớp nội dung trên mọi màn hình) */
.form-panel{
...
/* ===== Thanh trên cùng: banner nền đỏ nhạt, nút 3 gạch + tiêu đề căn giữa ===== */
.app-header {
...
/* Tên tài khoản đăng nhập: không còn khung/nền bọc, chỉ icon + chữ trắng nổi trên nền banner đỏ.
   flex-shrink cho phép co lại và luôn có max-width + ellipsis để không bao giờ lấn vào tiêu đề. */
.header-user-badge {
...
/* Khung thi chiếm gần hết chiều cao còn lại của màn hình, tự co giãn theo header/timer */
...
async function showHeaderUserBadge(){
    try{
        ...
        headerUserName.title = username; // xem đầy đủ khi tên bị cắt bớt (hover / giữ chạm)
        headerUserBadge.classList.remove("hidden");
    }catch(err){ /* bỏ qua nếu không lấy được, không ảnh hưởng chức năng thi */ }
}
...
async function refreshFileInfo(){
    ...
    if(noRepeatCheckbox.checked){
        totalText.textContent = `Tổng số câu hỏi: ${data.total} — Chưa thi: ${data.remaining} / Đã thi: ${data.total - data.remaining}`;
    } else {
...
async function backToSetup(){
    ...
    // Giữ nguyên bộ đề vừa thi (fileSelect không bị reset) và cập nhật lại
    // tổng số câu / số đã thi / số còn lại chưa thi cho bộ đề đó.
...
function showQuestion(i){
    ...
    q.options.forEach(opt=>{
        ...
        label.querySelector("input").addEventListener("change", ()=>{
            ...
            // Tô xanh đáp án đúng (dù chọn đúng hay sai), tô đỏ đáp án bạn chọn nếu sai
...
def add_months(source_date, months):
    """Cộng thêm 'months' tháng vào source_date, xử lý tràn ngày cuối tháng (vd 31/1 + 1 tháng -> 28/2 hoặc 29/2)."""
...
def is_expired_str(expires_at):
    """Kiểm tra một chuỗi hạn sử dụng (định dạng %Y-%m-%d %H:%M:%S) đã quá hạn hay chưa."""
...
def hash_password(password):
    """Băm mật khẩu có salt (werkzeug/pbkdf2) - an toàn hơn nhiều so với SHA-256 trần."""
    return generate_password_hash(password)


def verify_password(password, stored_hash):
    """Kiểm tra mật khẩu, tương thích ngược với hash SHA-256 cũ (không salt) đã lưu trước đây."""
    ...
    if stored_hash.startswith("pbkdf2:") or stored_hash.startswith("scrypt:"):
        try:
            ...
            return False
    # Định dạng cũ: sha256 hex thuần (64 ký tự), không salt
...
# Mật khẩu mặc định cho lần duyệt (approve) đầu tiên của mỗi tài khoản.
# Từ lần duyệt lại thứ 2 trở đi (huỷ rồi duyệt lại, hoặc duyệt lại nói chung) sẽ dùng mật khẩu ngẫu nhiên.
...
def login():
    ...
    if not (stored_password.startswith("pbkdf2:") or stored_password.startswith("scrypt:")):
        # Tự động nâng cấp hash cũ (sha256 trần) lên định dạng có salt ngay khi đăng nhập thành công
...
def logout():
    ...
    if device_id and email:
        ...
        if not row.empty and str(row.iloc[0]["active_device_id"] or "").strip() == device_id:
            ...
            save_devices(df)
    # Xóa tiến trình "không lặp câu đã thi" của phiên đăng nhập này
    if email:
...
def admin():
  def is_admin_request():
    # allow admin via cookie or via ?pwd (backwards compatible)
  ...
  # pass pwd for forms compatibility (keeps existing hidden inputs)
...
def admin_set_expiry():
  ...
  if not match.empty:
    ...
    now = datetime.now()
    if action == "clear":
      # Bỏ giới hạn -> tài khoản dùng không thời hạn
...
def parse_quiz_rows(df):
    """Đọc dataframe bộ đề, trả về danh sách câu hỏi hợp lệ kèm id (theo vị trí dòng gốc)."""
    items = []
...
def start_quiz():
    ...
    if no_repeat:
        ...
        if not available and total > 0:
            # Đã thi hết toàn bộ câu hỏi trong bộ đề này -> bắt đầu vòng ôn tập mới
            used_ids.clear()
...
if __name__ == "__main__":
    ...
    # LƯU Ý: server dev của Flask (app.run) chỉ phù hợp để chạy thử ở máy cá nhân.
    # Khi đưa lên internet thật, hãy chạy bằng WSGI server (vd: gunicorn app:app)
    # phía sau reverse proxy có HTTPS (Nginx/Caddy) - xem hướng dẫn triển khai đã gửi kèm.
