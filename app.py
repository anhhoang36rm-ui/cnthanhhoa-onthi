...
def hash_password(password):
    # QUAN TRỌNG: KHÔNG dùng scrypt (mặc định của werkzeug generate_password_hash).
    # scrypt cố tình được thiết kế tốn nhiều RAM/CPU để chống dò mật khẩu - trên các gói
    # hosting miễn phí bị giới hạn RAM thấp (vd Render free tier), việc này khiến tiến trình
    # bị hệ thống GIẾT CỨNG (SIGKILL) giữa chừng do "hết bộ nhớ", KHÔNG PHẢI một Exception
    # nên try/except không thể bắt và xử lý được. Dùng pbkdf2:sha256 thay thế - nhẹ hơn scrypt
    # rất nhiều lần, không tốn nhiều RAM, vẫn là thuật toán mã hoá mật khẩu an toàn tiêu chuẩn.
